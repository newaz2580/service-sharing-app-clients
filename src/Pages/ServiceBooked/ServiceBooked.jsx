...
const ServiceBooked = () => {
  ...
  useEffect(() => {
    if (user?.email && Array.isArray(allBookings)) {
      const filtered = allBookings.filter(
        (booking) =>
          booking.email === user.email ||
          booking.user_email === user.email ||
          booking.bookedUserEmail === user.email
      );
      setMyBookedServices(filtered);
    }
  }, [allBookings, user]);
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-700 px-4 py-10">
      ...
      {myBookedServices.length === 0 ? (
        <p className="text-center text-xl text-red-600 dark:text-red-400 font-semibold mt-20">
